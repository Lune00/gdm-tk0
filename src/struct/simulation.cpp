...
{ // Probleme ou pas pour la relecture? a voir ...
  ...
  while(datafile)
  {
    ...
    if (token == "GroupData{" || token == "includeGroups")
    {
      if (grpDat_ == 0) grpDat_ = new GroupData();

      if(token == "GroupData{") grpDat_->read(datafile);

      if(token == "includeGroups"){
	string filename;
	datafile >> filename;
	grpDat_->read(filename.c_str());
      }

    }

    if (token == "GroupRelationData{" || token == "includeGroupsRelations")
    {
      if (grpRel_ == 0) grpRel_ = new GroupRelationData();
      if (token == "GroupRelationData{") grpRel_->read(datafile);
      if (token == "includeGroupsRelations" ){
	string filename;
	datafile >> filename;
	grpRel_->read(filename.c_str());
      }
    }
