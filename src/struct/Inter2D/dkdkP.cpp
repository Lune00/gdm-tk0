...
void dkdkP::read(istream & is, unsigned int * Id1, unsigned int * Id2)
{
  is >> *Id1 >> *Id2
  >> nx_ >> ny_
  >> fn_ >> ft_ >> frot_ >> P_ ;
...
void dkdkP::write(ostream & os)
{
  os << "dkdkP " << i_->id() << ' ' << j_->id() << ' '
  << nx_ << ' ' << ny_ << ' ' 
  << fn_ << ' ' << ft_ << ' ' << frot_ << ' ' << P_
  << endl << flush;
}
...
double dkdkP::Dist()
{
  double lx = j_->x() + P_ - i_->x();
  double ly = j_->y() - i_->y();

  return sqrt(lx*lx+ly*ly) - i_->R() - j_->R();
}

double dkdkP::Overlap()
{
  double lx = j_->x() +  P_ - i_->x();
  double ly = j_->y() - i_->y();
...
void dkdkP::Kin()
{
	...
	else if ( i_->bodyDof()!= NULL && j_->bodyDof()== NULL )
	{
		...
		double cjx = x_ -j_->x() -P_;
		double cjy = y_ -j_->y();
		
		double vx = i_->bodyDof()->mcvx() - j_->vx() - ciy * i_->bodyDof()->mcvrot() + cjy * j_->vrot();// - j_->R() * j_->vrot();
		double vy = i_->bodyDof()->mcvy() - j_->vy() + cix * i_->bodyDof()->mcvrot() - cjx * j_->vrot();// - j_->R() * j_->vrot();
	...
	else if( i_->bodyDof()== NULL && j_->bodyDof()!= NULL ) 
	{
		...
		double cjx = x_ -j_->bodyDof()->mcx()-P_;
...
{		
	  ...
	  if (j_->bodyDof()==0)
	  {
		  ...
		  j_->bodyDof()->resDof(-fx,-fy ,-j_->R() * ft_ - frot_,x_-P_,y_);

}

void dkdkP::Res(const double dfn, const double dft, const double dfs)
{		
	...
	i_->bodyDof()->resDof(dfx,dfy,-i_->R() * dft + dfs,x_,y_);
	 ...
	 if (j_->bodyDof()==0)
	 {
	  ...
	  j_->bodyDof()->resDof(-dfx,-dfy,-j_->R() * dft - dfs,x_-P_,y_);
}

void dkdkP::CDcoeff() // CDcoeff(double en,double et) ou CDcoeff(grpRel&)
{
  	double mn, mt, ms;
...
double dkdkP::An(const double dt)
{
  	if( i_->bodyDof()==NULL && j_->bodyDof()==NULL  )
		return (
			- facn0_ * vn_/dt + fn_
		...
		{cout<<" pb dkdkP"<<endl;return 0;}
}

double dkdkP::At(const double dt)
{
  	if( i_->bodyDof()==NULL && j_->bodyDof()==NULL  )
		return (
			- fact0_ * vt_/dt + ft_
...
double dkdkP::As(const double dt)
{
  	if( i_->bodyDof()==NULL && j_->bodyDof()==NULL  )
		return (
...
void dkdkP::writeMGP(ostream & os)
{
