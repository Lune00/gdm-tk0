...
void GroupRelationData::read(const char * fname)
{
  ifstream datafile(fname);
  if(!datafile)
  {
    cerr<<"@GroupRelationData::read, cannot open file "<<fname<<endl;
    return ;
  }
  ...
  {	
	...
	{ 
	  ...
	  datafile >> parName >> g1 >> g2 >> value;
	  setParameter(parName,g1,g2,value);
	}
      }
      cerr<<"GroupRelationData defined using groups.ini."<<endl;
    }
    datafile >> token;
  }
}
