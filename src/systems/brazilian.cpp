...
void brazilian::read_parameters(istream & is)
{
	string token;
	string mode;
	...
	{	
		...
		else cerr << "@brazilian::read_parameters, Unknown parameter: " << token << endl;

		is >> token;
	}
}

void brazilian::write_parameters(ostream & os)
{
	os << "brazilian" << endl;
	os << "yval   " << yval_ << endl;
...
// Check the type of first four bodies
	for (unsigned int i=0;i<4;++i) 
		...
		gdm::fatal("@brazilian::init, the first four bodies must be rlines!");
...
// left is fixed
	lctrl_[idLeft_].x()        = _PRESSURE;
	lctrl_[idLeft_].y()        = _PRESSURE;
	lctrl_[idLeft_].rot()      = _PRESSURE;
...
// right
	...
	lctrl_[idRight_].x()       = _PRESSURE;
	lctrl_[idRight_].y()       = _PRESSURE;
	lctrl_[idRight_].rot()     = _PRESSURE;	
...
// When a pressure is applied, this is in fact a force from the algorithm point of view !!
	if (lctrl_[idBottom_].y() == _PRESSURE) lctrl_[idBottom_].y() = _FORCE;
...
void brazilian::drive() 
{
	...
	if (ymod_ == _PRESSURE)  
	{
		...
		lctrl_[idBottom_].yval() = -lctrl_[idTop_].yval();
	}
...
void brazilian::trans() 
{

}

void brazilian::share() 
{
